...
void setup_world(EntityGraph& ballgame, ResourceManager& ballpit)
{
	...
	//root
	auto view = make_shared<Camera2D>((coord)DEPTH);
	...
	view->children.push_back(make_shared<Obstacle>(box, LOC(250, 400), ROT(0,0,0)));
	view->children.push_back(make_shared<Obstacle>(box, LOC(500, 400), ROT(45,90,0), false));
	view->children.push_back(make_shared<Obstacle>(box, LOC(750, 400), ROT(0,0,0)));
	view->children.push_back(make_shared<Obstacle>(box, LOC(1000, 400), ROT(45,90,0), false));

	//fixed balls
	auto big = (coord)35;
	auto bigBall = [&](Colour4F c){ return ballpit.load_primitive((SolidColourBrush*)ballpit.load_brush(c), Primitive::Sphere, &big); };
	view->children.push_back(make_shared<Ball>(bigBall(Colour4F::CYAN), LOC(100,100), VEL(100,-100), ROT(0,0,0)));
	view->children.push_back(make_shared<Ball>(bigBall(Colour4F::MAGENTA), LOC(580,300), VEL(100,100), ROT(0,0,0)));
	view->children.push_back(make_shared<Ball>(bigBall(Colour4F::YELLOW), LOC(260,500), VEL(-100,100), ROT(0,0,0)));
